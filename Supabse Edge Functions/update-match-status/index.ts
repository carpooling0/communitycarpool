...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { data: match } = await supabase.from('matches').select(`
      ...
      sub_a:submissions!sub_a_id (submission_id, user_id, journey_num, from_location, to_location, users(name, email, match_page_token, email_whitelist, email_bounced)),
      sub_b:submissions!sub_b_id (submission_id, user_id, journey_num, from_location, to_location, users(name, email, match_page_token, email_whitelist, email_bounced))
